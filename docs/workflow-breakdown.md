...
Different inpainting checkpoints can produce different results. If the output does not match the image style, the checkpoint is one of the first things to test. The public workflow export uses a generic checkpoint placeholder instead of a machine-specific local model name.
