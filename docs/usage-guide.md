...
Start ComfyUI and load the sanitized workflow export:

```text
configs/comfyui-inpainting-workflow.json
```

## 2. Select An Inpainting Checkpoint

Use an SDXL checkpoint designed for inpainting. The public workflow uses a generic placeholder checkpoint name, so the checkpoint node should be pointed to the local inpainting model available in your ComfyUI installation.
